...
## Email setup (send reports as Excel)

Every "📧" icon across the dashboard — in the sidebar, and under Performance
Tracker, Scenario & Resilience, the Employee Portal's department comparison,
and the Company Profile — generates a formatted **Excel workbook** specific to
that section (KPIs and trend data, all four scenario modules, a department's
ranking table, or the full company profile) and sends it as an attachment via
SMTP. It needs a sending email account:
...
- The Daily log collects plain counts (units wasted, batches completed,
  batches on time, units failed QC) rather than asking employees to estimate
  percentages themselves. The app calculates wastage %, batch-time adherence
  %, and quality pass % automatically at submit time, and "My performance" /
  the manager view continue reporting in percentage form as before.

**If you already ran the Supabase SQL before this update**, add the four new
columns so shift-log submissions keep working:
```sql
alter table employee_performance add column units_wasted int;
alter table employee_performance add column batches_completed int;
alter table employee_performance add column batches_on_time int;
alter table employee_performance add column units_failed_qc int;
```
